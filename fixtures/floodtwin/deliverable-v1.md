...
Document version 1.4. Supersedes 1.3.
...
### 6.5 Risks and open decisions

| ID | Gap | Owner | Closure evidence |
|---|---|---|---|
| SF-G01 | Gap-fill method not selected | SF lead | Method comparison |
| SF-G02 | Quality flag taxonomy incomplete | SF lead | Taxonomy document |
| SF-G03 | Third-party platform rate limits unknown | SF lead | Operator confirmation |
| SF-G04 | Sensor clock skew tolerance | SF lead | Timing analysis |
| SF-G05 | Retention period for raw telemetry | Authority | Policy decision |
| SF-G06 | Revalidation window after dormancy | SF lead | Calibration note |
| SF-G07 | Disagreement flag propagation | SF lead | Interface note |
...
### 7.5 Risks and open decisions

| ID | Gap | Owner | Closure evidence |
|---|---|---|---|
| HM-G01 | Solver not selected | HM lead | Benchmark report |
| HM-G02 | Projection horizon not fixed | HM lead | Operational requirement |
| HM-G03 | Residual normalisation denominator undefined | HM lead | Method note |
| HM-G04 | Confidence propagation from SF flags | HM lead | Design note |
| HM-G05 | Behaviour when constraint set is infeasible | HM lead | Failure analysis |
...
Items named in this deliverable that are not yet defined:

| ID | Item |
|---|---|
| ND-01 | Tick duration |
| ND-02 | θ_surge value |
| ND-03 | Residual normalisation denominator |
| ND-04 | Projection horizon |
| ND-05 | Revalidation window |
| ND-06 | Alert suppression window |
| ND-07 | Raw telemetry retention period |
| ND-08 | Acceptance threshold for a fitted calibration vector |
...
|---|---|---|---|
| RSK-01 | Operator agreements not executed before build | Authority | Early legal engagement |
| RSK-02 | Historical archive insufficient for calibration | CP lead | Synthetic event augmentation |
| RSK-03 | Solver selection slips beyond design freeze | HM lead | Timeboxed benchmark |
| RSK-04 | Tick budget exceeded under peak load | RO lead | Load test before commit |
| RSK-05 | Threshold values not agreed in time | Authority | Provisional values with review |
...
Trade-off analyses for each subsystem are recorded in Appendix C. Retention
periods for every store are recorded in Section 21. Component
maturity assessments are recorded in Section 9. Open gaps are recorded in the
per-component registers. Risks are recorded in Appendix B.
